...
//上下半年
@property (nonatomic, strong) NSArray *partYearList;
@property (nonatomic, strong) NSArray *quarterList;
...
@property(nonatomic,copy)NSString*quarterString;
...
- (void)selectedDateLogic {
    switch (self.datePickerMode) {
        ...
        case PGDatePickerModeYearAndQuarter:
        {
            NSString *yearString = [self.pickerView textOfSelectedRowInComponent:0];
            yearString = [yearString componentsSeparatedByString:self.yearString].firstObject;
            
            NSString *quarterString = [self.pickerView textOfSelectedRowInComponent:1];
            quarterString = [quarterString componentsSeparatedByString:self.quarterString].firstObject;
            self.selectedComponents.year = [yearString integerValue];
            self.selectedComponents.month = [quarterString integerValue];
    ...
    switch (self.datePickerMode) {
        ...
        
        {
            if (component == 1) {
                ...
                return self.partYearList.count;
            }
            return self.yearList.count;
        }
        case PGDatePickerModeYearAndQuarter:
        ...
        case PGDatePickerModeYearAndWeek:
    ...
    switch (self.datePickerMode) {
        ...
        case PGDatePickerModePartYear:
        {
            ...
            [self.pickerView selectRow:row inComponent:0 animated:animated];
            {
                ...
                BOOL isExist = [self.partYearList containsObject:string];
                if (isExist) {
                    row = [self.partYearList indexOfObject:string];
                }
        ...
        case PGDatePickerModeYearAndQuarter:
        {
            ...
            [self.pickerView selectRow:row inComponent:0 animated:animated];
            {
                NSInteger row = 0;
                NSString *string = [NSString stringWithFormat:@"%ld", components.month];
                BOOL isExist = [self.quarterList containsObject:string];
                if (isExist) {
                    row = [self.quarterList indexOfObject:string];
                }
                [self.pickerView selectRow:row inComponent:1 animated:animated];
            }
        }
        ...
        case PGDatePickerModeYearAndWeek:
        {
            ...
            [self.pickerView selectRow:row inComponent:0 animated:animated];
            {
                NSInteger row = 0;
                NSString *string = [NSString stringWithFormat:@"%ld", components.month];
                BOOL isExist = [self.weekList containsObject:string];
                if (isExist) {
                    row = [self.weekList indexOfObject:string];
                }
                [self.pickerView selectRow:row inComponent:1 animated:animated];
            }
        }
...
- (NSString *)pickerView:(PGPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    switch (self.datePickerMode) {
        ...
        {
            if (component == 1) {
                return [self.partYearList[row] stringByAppendingString:self.middleYearString];
            }
            return [self.yearList[row] stringByAppendingString:self.yearString];
        }
        ...
        {
            if (component == 1) {
                return [self.quarterList[row] stringByAppendingString:self.quarterString];
            }
            return [self.yearList[row] stringByAppendingString:self.yearString];
        }
        case PGDatePickerModeYearAndWeek:
        {
            if (component == 1) {
                return [self.weekList[row] stringByAppendingString:self.weekString];
            }
            return [self.yearList[row] stringByAppendingString:self.yearString];
        }
...
-(NSArray *)partYearList
{
    
    if (!_partYearList)
    {
        NSMutableArray *partYears = [NSMutableArray arrayWithArray:@[@"上半",@"下半"]];
        _partYearList = partYears;
    }
    
    return _partYearList;
}
...
-(NSArray *)quarterList
{
    ...
    if (!_quarterList)
    {
        //NSMutableArray *quarterList = [NSMutableArray arrayWithArray:@[@"第一季度",@"第二季度"，@"第三季度",@"第四季度"]];
        
        NSMutableArray *quarterList = [NSMutableArray arrayWithArray:@[@"1",@"2",@"3",@"4"]];
        _quarterList = quarterList;
    }
    return _quarterList;
}
...
-(NSArray *)weekList
{
    ...
    if (!_weekList)
    {
        ...
        for (int i = 0;i <53;i++)
        {
           ...
           [weekList addObject:[@(i+1) stringValue]];
...
-(NSString *)quarterString
{
    ...
    if (!_quarterString) {
        _quarterString = [NSBundle localizedStringForKey:@"quarterString"];
    }
...
{
    if (!_weekString) {
        _weekString = [NSBundle localizedStringForKey:@"weekString"];
    }
    return _weekString;
