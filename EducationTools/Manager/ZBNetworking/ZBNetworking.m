...
- (ZBURLSessionTask *)uploadFileWithUrl:(NSString *)url fileData:(NSData *)data name:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType progressBlock:(ZBUploadProgressBlock)progressBlock successBlock:(ZBResponseSuccessBlock)successBlock failBlock:(ZBResponseFailBlock)failBlock {
    ...
    session = [manager POST:url parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        [NetShowHelper showTip:responseObject];
        [requestTasksPool removeObject:session];
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        if (failBlock) failBlock(error);
        [requestTasksPool removeObject:session];
...
#pragma mark json数据上传
- (ZBURLSessionTask *)uploadJsonDataWithUrl:(NSString *)url jsonData:(NSData *)data fileName:(NSString *)fileName  progressBlock:(ZBUploadProgressBlock)progressBlock successBlock:(ZBResponseSuccessBlock)successBlock failBlock:(ZBResponseFailBlock)failBlock {
    __block ZBURLSessionTask *session = nil;
    AFHTTPSessionManager *manager = [self manager];
    if (networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
        if (failBlock) failBlock(ZB_ERROR);
    ...
    if (app.token) {
        [manager.requestSerializer setValue:app.token forHTTPHeaderField:@"User-Token"];
    }
    session = [manager POST:url parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
#pragma mark json数据上传
- (ZBURLSessionTask *)uploadJsonDataWithUrl:(NSString *)url jsonData:(NSData *)data jsonName:(NSString *)name file:(NSData *)fileData fileName:(NSString *)fileName fileType:(NSString *)type progressBlock:(ZBUploadProgressBlock)progressBlock successBlock:(ZBResponseSuccessBlock)successBlock failBlock:(ZBResponseFailBlock)failBlock {
    __block ZBURLSessionTask *session = nil;
    AFHTTPSessionManager *manager = [self manager];
    if (networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
        if (failBlock) failBlock(ZB_ERROR);
